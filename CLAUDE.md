...
src/data/<battle>.ts     One battle = one data file (currently: ladon.ts)
src/engine/timeline.ts   Pure functions: position/strength of any unit at time t
src/components/          MapCanvas (canvas map), TimelineControls, EventLog, InfoPanel
src/App.tsx              Shell + playback loop (battle currently hardcoded to `ladon`)
docs/                    Research knowledge base (see below)
```

- Units have **waypoint paths** and **strength timelines**; both are linearly interpolated by `engine/timeline.ts`. Hazards (storms, waves) are units with `kind: 'hazard'` and an intensity 1–5 as "strength".
- `MapCanvas` renders map features by `kind` (`sea`, `desert`, `mountains`, `city`, `plains`, `landmark`, `route`), then hazards, then units, then event pulses (events within the last 1.5 battle-hours).
- Known TODO: a battle selector (App hardcodes one battle); pan/zoom; casualty graphs; a space-battle map mode (needed for Ilium — no terrain, 3D-ish ranges).
...
2. **Battle of Ilium** (Aug 743, Jupiter) — needs the space-battle map mode
3. **The Fall of Luna** (Nov 743) — fleet + citadel assault
4. **Siege of Phobos** (Nov–Dec 754) — multi-Iron-Rain station siege
5. Engine: battle selector, unit filtering, casualty graphs, pan/zoom
